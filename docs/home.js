// ======================
// MAGICAL WAVE CANVAS (LIVE WAVES + SPARKLES)
// ======================
...
// ======================
// HOME.JS - ALL FUNCTIONALITY PRESERVED
// ======================
...
function announceWelcome() {
    ...
    welcomeSpoken = true;
    const allItems = document.querySelectorAll(".tappable");
    const labels = Array.from(allItems).map(el => el.dataset.label);
    speak("Welcome to Askari Bank Home Page. " + (labels.length ? "Available options: " + labels.join(". ") : "") + ". Triple tap on any box to hear its name. Single tap to select.");
}
function initWelcomeTrigger() {
    const trigger = () => { if (speechSynthesis.getVoices().length) announceWelcome(); else speechSynthesis.onvoiceschanged = announceWelcome; document.removeEventListener("touchstart", trigger); document.removeEventListener("click", trigger); };
    document.addEventListener("touchstart", trigger, { once: true });
    document.addEventListener("click", trigger, { once: true });
}

function initTripleTap() {
    document.querySelectorAll(".tappable").forEach(item => {
        let tapCount = 0, tapTimer = null;
        item.addEventListener("click", (e) => { e.stopPropagation();
            tapCount++;
            if (tapTimer) clearTimeout(tapTimer);
            tapTimer = setTimeout(() => {
                const label = item.dataset.label || "option", page = item.dataset.page;
                if (tapCount >= 3) {
                    document.querySelectorAll(".tappable").forEach(i => i.classList.remove("triple-tap-focus"));
                    item.classList.add("triple-tap-focus");
                    setTimeout(() => item.classList.remove("triple-tap-focus"), 2000);
                    speak(label + ". Tap once to select.");
                } else if (tapCount === 2) speak(label);
                else {
                    speak(label + " selected.");
                    if (page && page !== "#") setTimeout(() => window.location.href = page, 1400);
                }
                tapCount = 0;
            }, 600);
        });
...
function setLoginTime() {
    const now = new Date();
    const formatted = now.toLocaleString("en-GB", { day: "2-digit", month: "2-digit", year: "numeric", hour: "2-digit", minute: "2-digit", hour12: false }).replace(",", "");
    const el = document.getElementById("loginTime");
    if (el) el.textContent = `Login at ${formatted}`;
}
function displayUserName() {
    const user = JSON.parse(localStorage.getItem("loggedInUser"));
    const welcomeLabel = document.querySelector(".welcome-label");
    if (user && welcomeLabel) {
        welcomeLabel.textContent = "Welcome Back, " + user.fullName.split(" ")[0];
    }
}
function initAccountNumberToggle() {
    const toggle = document.getElementById("toggleAccountNumber");
    const accEl = document.getElementById("accountNumber");
    const user = JSON.parse(localStorage.getItem("loggedInUser"));
    const real = user?.accountNumber || "3001-2345-6789";
    const masked = "●●●● ●●●● ●●●●";
    let hidden = true;
    if(toggle && accEl) {
        accEl.textContent = masked;
        toggle.addEventListener("click", () => { 
            hidden = !hidden; 
            accEl.textContent = hidden ? masked : real; 
            toggle.innerHTML = hidden ? '<i class="fas fa-eye-slash"></i>' : '<i class="fas fa-eye"></i>'; 
            speak(hidden ? "Account number hidden" : "Account number is " + real); 
        });
    }
}
function initBalanceToggle() {
    ...
    const user = JSON.parse(localStorage.getItem("loggedInUser"));
    const real = user?.balance?.toLocaleString() || "1,25,430";
    const masked = "X X X X X X";
    let hidden = true;
    if(toggle && balEl) {
        balEl.textContent = masked;
        toggle.addEventListener("click", () => { 
            hidden = !hidden; 
            balEl.textContent = hidden ? masked : real; 
            toggle.innerHTML = hidden ? '<i class="fas fa-eye-slash"></i>' : '<i class="fas fa-eye"></i>'; 
            const balanceWords = real.replace(/,/g, "");
            speak(hidden ? "Balance hidden" : "Balance is " + balanceWords + " rupees"); 
        });
    }
}
function initAssistToggle() {
    const toggle = document.getElementById("assistToggle");
    const sub = document.querySelector(".assist-sub");
    if(toggle) toggle.addEventListener("change", () => { if(toggle.checked) { if(sub) sub.textContent = "Voice Assistant is On"; speak("Voice assistant enabled"); } else { if(sub) sub.textContent = "Voice Assistant is Off"; window.speechSynthesis.cancel(); setCaption("Voice assistant is off"); } });
}
function initPowerBtn() {
    const btn = document.getElementById("powerBtn");
    if(btn) btn.addEventListener("click", () => { speak("Logging out. Goodbye."); setTimeout(() => window.location.href = "index.html", 1200); });
}
function initMenuBtn() {
    const btn = document.getElementById("menuBtn");
    if(btn) btn.addEventListener("click", () => { speak("Opening settings."); setTimeout(() => window.location.href = "settings.html", 800); });
}
function initProfileNavBtn() {
    const btn = document.getElementById("profileNavBtn");
    if(btn) btn.addEventListener("click", () => { speak("Opening profile."); setTimeout(() => window.location.href = "myProfile.html", 800); });
}
function initVoiceTip() {
    const tip = document.getElementById("voiceTip");
    if(tip) tip.addEventListener("click", () => tip.style.display = "none");
}
function initBottomNav() {
    document.querySelectorAll(".nav-btn").forEach(btn => { btn.addEventListener("click", () => { document.querySelectorAll(".nav-btn").forEach(b => b.classList.remove("active")); btn.classList.add("active"); }); });
    const reqPay = document.getElementById("requestPayBtn");
    if(reqPay) reqPay.addEventListener("click", () => speak("Request to pay feature coming soon."));
    const miniStmt = document.getElementById("miniStmtBtn");
    if(miniStmt) miniStmt.addEventListener("click", () => speak("Mini statement feature is under development."));
    const share = document.getElementById("shareBtn");
    if(share) share.addEventListener("click", () => speak("Share account details feature will be available soon."));
    const edit = document.getElementById("editFreqBtn");
    if(edit) edit.addEventListener("click", () => speak("Edit frequently used items coming soon."));
    const viewAll = document.getElementById("viewAllBtn");
    if(viewAll) viewAll.addEventListener("click", () => speak("View all services will open in future update."));
}
document.addEventListener("DOMContentLoaded", () => {
    setLoginTime();
    initAccountNumberToggle();
    displayUserName();
    initBalanceToggle();
    initAssistToggle();
    initPowerBtn();
