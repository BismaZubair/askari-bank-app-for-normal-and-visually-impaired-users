...
// ================================================
// SETTINGS JS - WITH VOICE TOGGLE & TRIPLE TAP
// ================================================
...
// Direct click handler - works immediately without waiting for welcomeDone
function initDirectClicks() {
    const items = document.querySelectorAll(".settings-item.tappable");
    items.forEach(item => {
        const handler = (e) => {
            e.stopPropagation();
...
async function runWelcome() {
    ...
    const items = document.querySelectorAll(".settings-item.tappable");
    const labels = Array.from(items).map(el => el.dataset.label);
    await speak("Settings page. Welcome. Your personal settings options are: " + labels.join(". ") + ". Triple tap on any option to hear its name. Single tap to select and open it. Use the toggle switch to turn voice assistant on or off. Tap the Done button to save and return.");
    welcomeDone = true;
}
...
function initWelcomeTrigger() {
    ...
    const handler = () => {
        ...
        if (voices.length > 0) {
            ...
            window.speechSynthesis.onvoiceschanged = () => runWelcome();
        }
    };
        ...
        item.addEventListener("click", (e) => {
            ...
            itemTimer = setTimeout(() => {
                ...
                if (count >= 3) {
                    ...
                    if (voiceEnabled) speak(label + ". Tap once to open.");
                } else if (count === 2) {
...
function initDoneBtn() {
    ...
    doneBtn.addEventListener("click", () => {
        if (voiceEnabled) {
            speak("Settings saved. Going back to home.").then(() => {
                window.location.href = "home.html";
            });
        } else {
            window.location.href = "home.html";
...
function initAssistToggle() {
    ...
    toggle.addEventListener("change", () => {
        ...
        if (voiceEnabled) {
            ...
            localStorage.setItem("voiceAssistant", "on");
            setCaption("Voice assistant enabled.");
            speak("Voice assistant enabled.");
        } else {
            if (subText) subText.textContent = "Voice Assistant is Off";
            localStorage.setItem("voiceAssistant", "off");
            window.speechSynthesis.cancel();
