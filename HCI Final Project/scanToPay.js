// ═══════════════════════════════════════════
//  MAGICAL WAVE CANVAS SYSTEM (SAME AS INDEX)
// ═══════════════════════════════════════════
(function initMagicalWaves() {
    const canvas = document.getElementById("waveCanvas");
    if (!canvas) return;
    const ctx = canvas.getContext("2d");
    let W, H;
    let time = 0;
    let sparkles = [];

    function resize() {
        W = canvas.width = window.innerWidth;
        H = canvas.height = window.innerHeight;
    }
    window.addEventListener("resize", resize);
    resize();

    function getWavePath(band, t, flip) {
        const points = [];
        const steps = 120;
        for (let i = 0; i <= steps; i++) {
            const x = (i / steps) * W;
            const nx = x / W;
            let y = Math.sin(nx * Math.PI * 3.2 + t * band.speed) * band.amp
                + Math.sin(nx * Math.PI * 5.1 + t * band.speed * 1.4 + 1.2) * band.amp * 0.5
                + Math.sin(nx * Math.PI * 7.7 + t * band.speed * 0.7 + 2.4) * band.amp * 0.25;
            const baseY = flip ? band.baseY : H - band.baseY;
            points.push({ x, y: flip ? baseY + y : baseY - y });
        }
        return points;
    }

    const bottomBands = [
        { baseY: 90, amp: 55, speed: 0.95, colorA: "rgba(0,200,255,0.55)", glowColor: "#00c8ff", lineW: 2.2 },
        { baseY: 110, amp: 28, speed: 0.45, colorA: "rgba(0,180,255,0.35)", glowColor: "#00b4ff", lineW: 1.4 },
        { baseY: 70, amp: 34, speed: 0.72, colorA: "rgba(80,220,255,0.70)", glowColor: "#50dcff", lineW: 2.8 },
        { baseY: 130, amp: 40, speed: 0.66, colorA: "rgba(0,140,230,0.20)", glowColor: "#008ce6", lineW: 1.0 }
    ];
    const topBands = [
        { baseY: 90, amp: 55, speed: 0.95, colorA: "rgba(0,200,255,0.50)", glowColor: "#00c8ff", lineW: 2.2 },
        { baseY: 110, amp: 28, speed: 0.45, colorA: "rgba(0,180,255,0.30)", glowColor: "#00b4ff", lineW: 1.4 },
        { baseY: 70, amp: 34, speed: 0.72, colorA: "rgba(80,220,255,0.65)", glowColor: "#50dcff", lineW: 2.8 },
        { baseY: 130, amp: 40, speed: 0.66, colorA: "rgba(0,140,230,0.18)", glowColor: "#008ce6", lineW: 1.0 }
    ];

    function drawWaveLine(points, band) {
        ctx.save();
        ctx.shadowBlur = 22;
        ctx.shadowColor = band.glowColor;
        ctx.strokeStyle = band.colorA;
        ctx.lineWidth = band.lineW + 4;
        ctx.globalAlpha = 0.25;
        ctx.beginPath();
        ctx.moveTo(points[0].x, points[0].y);
        for (let i = 1; i < points.length; i++) {
            const prev = points[i - 1];
            const curr = points[i];
            const mx = (prev.x + curr.x) / 2;
            const my = (prev.y + curr.y) / 2;
            ctx.quadraticCurveTo(prev.x, prev.y, mx, my);
        }
        ctx.stroke();
        ctx.restore();
        ctx.save();
        ctx.shadowBlur = 14;
        ctx.shadowColor = band.glowColor;
        ctx.strokeStyle = band.colorA;
        ctx.lineWidth = band.lineW;
        ctx.beginPath();
        ctx.moveTo(points[0].x, points[0].y);
        for (let i = 1; i < points.length; i++) {
            const prev = points[i - 1];
            const curr = points[i];
            const mx = (prev.x + curr.x) / 2;
            const my = (prev.y + curr.y) / 2;
            ctx.quadraticCurveTo(prev.x, prev.y, mx, my);
        }
        ctx.stroke();
        ctx.restore();
    }

    function drawWaveFill(points, flip) {
        ctx.save();
        ctx.globalAlpha = 0.12;
        const grad = ctx.createLinearGradient(0, flip ? 0 : H, 0, flip ? H * 0.35 : H * 0.65);
        grad.addColorStop(0, "rgba(0,180,255,0.55)");
        grad.addColorStop(1, "rgba(0,0,0,0)");
        ctx.fillStyle = grad;
        ctx.beginPath();
        ctx.moveTo(points[0].x, points[0].y);
        for (let i = 1; i < points.length; i++) {
            const prev = points[i - 1];
            const curr = points[i];
            const mx = (prev.x + curr.x) / 2;
            const my = (prev.y + curr.y) / 2;
            ctx.quadraticCurveTo(prev.x, prev.y, mx, my);
        }
        if (flip) { ctx.lineTo(W, 0); ctx.lineTo(0, 0); }
        else { ctx.lineTo(W, H); ctx.lineTo(0, H); }
        ctx.closePath();
        ctx.fill();
        ctx.restore();
    }

    function spawnSparkle(x, y, color) {
        sparkles.push({ x, y, vx: (Math.random() - 0.5) * 1.2, vy: (Math.random() - 0.5) * 1.2, radius: Math.random() * 2.4 + 0.6, life: 1.0, decay: Math.random() * 0.025 + 0.012, color });
    }
    function spawnSparklesOnWave(points, band) {
        const count = Math.floor(Math.random() * 3) + 1;
        for (let i = 0; i < count; i++) {
            const idx = Math.floor(Math.random() * points.length);
            const p = points[idx];
            spawnSparkle(p.x, p.y + (Math.random() - 0.5) * 10, band.glowColor);
        }
    }
    function drawSparkles() {
        for (let i = sparkles.length - 1; i >= 0; i--) {
            const s = sparkles[i];
            s.x += s.vx; s.y += s.vy; s.life -= s.decay;
            if (s.life <= 0) { sparkles.splice(i, 1); continue; }
            ctx.save(); ctx.globalAlpha = s.life; ctx.shadowBlur = 10; ctx.shadowColor = s.color;
            ctx.strokeStyle = s.color; ctx.lineWidth = s.radius * 0.6;
            ctx.beginPath(); ctx.moveTo(s.x - s.radius * 2, s.y); ctx.lineTo(s.x + s.radius * 2, s.y);
            ctx.moveTo(s.x, s.y - s.radius * 2); ctx.lineTo(s.x, s.y + s.radius * 2);
            ctx.moveTo(s.x - s.radius, s.y - s.radius); ctx.lineTo(s.x + s.radius, s.y + s.radius);
            ctx.moveTo(s.x + s.radius, s.y - s.radius); ctx.lineTo(s.x - s.radius, s.y + s.radius);
            ctx.stroke();
            ctx.fillStyle = "#ffffff"; ctx.shadowBlur = 6;
            ctx.beginPath(); ctx.arc(s.x, s.y, s.radius * 0.4, 0, Math.PI * 2); ctx.fill();
            ctx.restore();
        }
    }

    const stars = [];
    for (let i = 0; i < 80; i++) stars.push({ x: Math.random(), y: Math.random(), r: Math.random() * 1.2 + 0.3, blink: Math.random() * Math.PI * 2, speed: Math.random() * 0.02 + 0.005 });
    function drawStars(t) {
        for (const s of stars) {
            const alpha = 0.3 + 0.5 * Math.abs(Math.sin(s.blink + t * s.speed));
            ctx.save(); ctx.globalAlpha = alpha; ctx.fillStyle = "#ffffff"; ctx.shadowBlur = 4; ctx.shadowColor = "#88ddff";
            ctx.beginPath(); ctx.arc(s.x * W, s.y * H, s.r, 0, Math.PI * 2); ctx.fill(); ctx.restore();
        }
    }

    function render() {
        ctx.clearRect(0, 0, W, H);
        drawStars(time);
        bottomBands.forEach((band, idx) => {
            const pts = getWavePath(band, time + idx * 1.1, false);
            drawWaveFill(pts, false);
            drawWaveLine(pts, band);
            if (band.lineW >= 2 && Math.random() < 0.6) spawnSparklesOnWave(pts, band);
        });
        topBands.forEach((band, idx) => {
            const pts = getWavePath(band, time + idx * 0.9 + 3.5, true);
            drawWaveFill(pts, true);
            drawWaveLine(pts, band);
            if (band.lineW >= 2 && Math.random() < 0.6) spawnSparklesOnWave(pts, band);
        });
        drawSparkles();
        if (sparkles.length > 400) sparkles.splice(0, 100);
        time += 0.025;
        requestAnimationFrame(render);
    }
    render();
})();

// ================================================
// SCAN TO PAY JS - WITH ASSISTPAY TOGGLE
// ================================================
let voiceEnabled = true;
let welcomeDone = false;
let cameraStream = null;
let flashOn = false;
let scanning = false;
let scanLoopId = null;
let jsQR = null;

function setCaption(text) {
    const el = document.getElementById("captionText");
    if (el) el.textContent = text;
}

function speak(text) {
    if (!voiceEnabled) return Promise.resolve();
    
    return new Promise((resolve) => {
        setCaption(text);
        window.speechSynthesis.cancel();
        const msg = new SpeechSynthesisUtterance(text);
        msg.rate = 0.88;
        msg.pitch = 1;
        msg.volume = 1;
        const voices = window.speechSynthesis.getVoices();
        const preferred = voices.find(v => v.lang === "en-US") || voices[0];
        if (preferred) msg.voice = preferred;
        msg.onend = () => resolve();
        msg.onerror = () => resolve();
        window.speechSynthesis.speak(msg);
    });
}

function setStatus(text, type) {
    const el = document.getElementById("scanStatus");
    if (!el) return;
    el.textContent = text;
    el.className = "scan-status visible " + (type || "");
}

function clearStatus() {
    const el = document.getElementById("scanStatus");
    if (el) el.className = "scan-status";
}

function setViewfinderState(state) {
    const vf = document.getElementById("viewfinder");
    if (vf) vf.className = "viewfinder " + state;
}

function loadJsQR() {
    return new Promise((resolve) => {
        if (window.jsQR) { jsQR = window.jsQR; resolve(); return; }
        const script = document.createElement("script");
        script.src = "https://cdnjs.cloudflare.com/ajax/libs/jsQR/1.4.0/jsQR.min.js";
        script.onload = () => { jsQR = window.jsQR; resolve(); };
        script.onerror = () => resolve();
        document.head.appendChild(script);
    });
}

async function startCamera() {
    const video = document.getElementById("cameraFeed");
    const overlay = document.getElementById("cameraOverlay");
    setCaption("Starting camera…");
    try {
        cameraStream = await navigator.mediaDevices.getUserMedia({
            video: { facingMode: "environment", width: { ideal: 1280 }, height: { ideal: 720 } },
            audio: false
        });
        video.srcObject = cameraStream;
        await video.play();
        overlay.classList.add("hidden");
        startScanLoop();
        if (voiceEnabled) speak("Camera started. Point the QR code inside the blue frame to scan and pay.");
    } catch (err) {
        console.error("Camera error:", err);
        overlay.classList.remove("hidden");
        if (voiceEnabled) {
            if (err.name === "NotAllowedError") {
                speak("Camera permission denied. Please allow camera access. You can also use the Scan From Gallery button.");
            } else {
                speak("Could not start the camera. Please use the Scan From Gallery button.");
            }
        }
        setStatus("Camera unavailable", "error");
    }
}

function stopCamera() {
    if (cameraStream) {
        cameraStream.getTracks().forEach(t => t.stop());
        cameraStream = null;
    }
    stopScanLoop();
}

const canvas = document.createElement("canvas");
const ctx = canvas.getContext("2d");

function startScanLoop() {
    if (scanning) return;
    scanning = true;
    setViewfinderState("scanning");
    setStatus("Scanning…", "scanning");
    if (voiceEnabled) setCaption("Scanning for QR code…");
    scanLoop();
}

function stopScanLoop() {
    scanning = false;
    if (scanLoopId) { cancelAnimationFrame(scanLoopId); scanLoopId = null; }
}

function scanLoop() {
    if (!scanning) return;
    const video = document.getElementById("cameraFeed");
    if (video.readyState === video.HAVE_ENOUGH_DATA && jsQR) {
        canvas.width = video.videoWidth;
        canvas.height = video.videoHeight;
        ctx.drawImage(video, 0, 0, canvas.width, canvas.height);
        const imageData = ctx.getImageData(0, 0, canvas.width, canvas.height);
        const code = jsQR(imageData.data, imageData.width, imageData.height, { inversionAttempts: "dontInvert" });
        if (code) {
            onQRDetected(code.data);
            return;
        }
    }
    scanLoopId = requestAnimationFrame(scanLoop);
}

function onQRDetected(data) {
    scanning = false;
    stopScanLoop();
    setViewfinderState("success");
    clearStatus();
    setStatus("QR Code Detected!", "success");
    const isUrl = data.startsWith("http://") || data.startsWith("https://");
    const isPayment = data.toLowerCase().includes("pay") || data.toLowerCase().includes("upi") || data.toLowerCase().includes("askari");
    if (isUrl || isPayment) {
        if (voiceEnabled) {
            speak("Q R code scanned successfully. Payment QR detected. Proceeding to confirm payment.").then(() => {
                sessionStorage.setItem("scannedQR", data);
                window.location.href = "scanPayConfirm.html";
            });
        } else {
            sessionStorage.setItem("scannedQR", data);
            window.location.href = "scanPayConfirm.html";
        }
        setTimeout(() => {
            sessionStorage.setItem("scannedQR", data);
            window.location.href = "scanPayConfirm.html";
        }, 3000);
    } else {
        if (voiceEnabled) {
            speak("Q R code scanned. This does not appear to be a payment QR code. Please try a different QR code.");
        }
        setStatus("Not a payment QR", "error");
        setViewfinderState("error");
        setTimeout(() => {
            setViewfinderState("scanning");
            clearStatus();
            startScanLoop();
            if (voiceEnabled) speak("Resuming scan. Please point the camera at a valid payment QR code.");
        }, 4000);
    }
}

function initGalleryBtn() {
    const btn = document.getElementById("galleryBtn");
    const input = document.getElementById("galleryInput");
    if (!btn || !input) return;
    btn.addEventListener("click", (e) => {
        e.stopPropagation();
        if (voiceEnabled) speak("Opening gallery. Please select an image containing a QR code.");
        setTimeout(() => input.click(), 800);
    });
    input.addEventListener("change", async () => {
        const file = input.files[0];
        if (!file) return;
        if (voiceEnabled) speak("Image selected. Scanning for QR code.");
        setViewfinderState("scanning");
        setStatus("Reading image…", "scanning");
        const img = new Image();
        img.onload = () => {
            canvas.width = img.width;
            canvas.height = img.height;
            ctx.drawImage(img, 0, 0);
            const imageData = ctx.getImageData(0, 0, canvas.width, canvas.height);
            if (!jsQR) {
                if (voiceEnabled) speak("QR scanner not loaded. Please check your internet connection.");
                setStatus("Scanner not ready", "error");
                return;
            }
            const code = jsQR(imageData.data, imageData.width, imageData.height, { inversionAttempts: "attemptBoth" });
            if (code) {
                onQRDetected(code.data);
            } else {
                setViewfinderState("error");
                setStatus("No QR found in image", "error");
                if (voiceEnabled) speak("No QR code found in the selected image.");
                setTimeout(() => {
                    setViewfinderState("scanning");
                    clearStatus();
                    if (cameraStream) startScanLoop();
                }, 3500);
            }
        };
        img.onerror = () => {
            if (voiceEnabled) speak("Could not read the image. Please try a different image.");
            setStatus("Image error", "error");
        };
        img.src = URL.createObjectURL(file);
        input.value = "";
    });
}

function initFlashBtn() {
    const btn = document.getElementById("flashBtn");
    if (!btn) return;
    btn.addEventListener("click", async (e) => {
        e.stopPropagation();
        if (!cameraStream) {
            if (voiceEnabled) speak("Camera is not active. Please wait for the camera to start.");
            return;
        }
        const track = cameraStream.getVideoTracks()[0];
        if (!track) return;
        try {
            flashOn = !flashOn;
            await track.applyConstraints({ advanced: [{ torch: flashOn }] });
            btn.classList.toggle("flash-on", flashOn);
            if (voiceEnabled) speak(flashOn ? "Flash on." : "Flash off.");
        } catch {
            if (voiceEnabled) speak("Flash is not supported on this device.");
        }
    });
}

async function runWelcome() {
    if (!voiceEnabled) return;
    await speak("Scan To Pay page. This page uses your camera to scan a QR code and process a payment. The camera will start automatically. Point the QR code inside the blue frame shown on screen. The app will detect it automatically. You can also tap the Scan From Gallery button to upload a QR code image. Use the flash button to turn on the torch. Double tap to go back.");
    welcomeDone = true;
    await loadJsQR();
    startCamera();
}

function initWelcomeTrigger() {
    let triggered = false;
    const handler = () => {
        if (triggered || !voiceEnabled) return;
        triggered = true;
        const voices = window.speechSynthesis.getVoices();
        if (voices.length > 0) runWelcome();
        else window.speechSynthesis.onvoiceschanged = () => runWelcome();
    };
    document.addEventListener("click", handler);
    document.addEventListener("touchstart", handler);
}

function initScreenTap() {
    const container = document.querySelector(".app-container");
    let tapCount = 0;
    let tapTimer = null;
    container.addEventListener("click", (e) => {
        if (!welcomeDone) return;
        if (e.target.closest(".gallery-btn") || e.target.closest(".back-btn") || e.target.closest(".cam-ctrl-btn")) return;
        tapCount++;
        if (tapTimer) clearTimeout(tapTimer);
        tapTimer = setTimeout(() => {
            const count = tapCount;
            tapCount = 0;
            if (count >= 2) {
                if (voiceEnabled) {
                    speak("Going back.").then(() => {
                        stopCamera();
                        window.location.href = "home.html";
                    });
                } else {
                    stopCamera();
                    window.location.href = "home.html";
                }
            } else {
                if (voiceEnabled) speak("Point the QR code inside the blue frame. The scan is automatic. Double tap to go back.");
            }
        }, 400);
    });
}

function initBackBtn() {
    const btn = document.getElementById("backBtn");
    if (!btn) return;
    btn.addEventListener("click", () => {
        if (voiceEnabled) {
            speak("Going back.").then(() => {
                stopCamera();
                window.location.href = "home.html";
            });
        } else {
            stopCamera();
            window.location.href = "home.html";
        }
    });
}

function initAssistToggle() {
    const toggle = document.getElementById("assistToggle");
    const subText = document.querySelector(".assist-sub");
    if (!toggle) return;
    toggle.addEventListener("change", () => {
        voiceEnabled = toggle.checked;
        if (voiceEnabled) {
            if (subText) subText.textContent = "Voice Assistant is On";
            setCaption("Voice assistant enabled. Tap anywhere to begin.");
            speak("Voice assistant enabled.");
        } else {
            if (subText) subText.textContent = "Voice Assistant is Off";
            window.speechSynthesis.cancel();
            setCaption("Voice assistant is off");
        }
    });
}

document.addEventListener("visibilitychange", () => {
    if (document.hidden) stopCamera();
});

document.addEventListener("DOMContentLoaded", () => {
    initAssistToggle();
    initBackBtn();
    initFlashBtn();
    initGalleryBtn();
    initScreenTap();
    initWelcomeTrigger();
    window.speechSynthesis.getVoices();
});